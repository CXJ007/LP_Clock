...
void AppMain(void)
{
    ...
    RetVal |= (Std_ReturnType)tx_thread_create(
        ...
        APP_CFG_TASK_MODE_CTRL_PRIO, APP_CFG_TASK_MODE_CTRL_PRIO,
        TX_NO_TIME_SLICE, TX_AUTO_START);

    RetVal |= (Std_ReturnType)tx_thread_create(
        &AppTask10msTCB, "App Task 10ms", AppTask10ms, 0U, AppTask10msStk,
        APP_CFG_TASK_10MS_STK_SIZE, APP_CFG_TASK_10MS_PRIO,
        APP_CFG_TASK_10MS_PRIO, TX_NO_TIME_SLICE, TX_AUTO_START);
...
static void AppTaskModeCtrl(ULONG thread_input)
{
    ...
    while(1)
    {
        ...
        RetVal = (Std_ReturnType)tx_event_flags_get(&gApp_ModeCtrlEventGroup, 
                0x00000001U, TX_OR_CLEAR, (ULONG*)&AppCtrlCmd, TX_WAIT_FOREVER);
        if(E_OK == RetVal)
        {
